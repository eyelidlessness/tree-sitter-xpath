...
const UNICODE_LETTER = /\p{L}/,
  ...
  DECIMAL = seq(repeat(UNICODE_DIGIT), token.immediate('.'), repeat1(token.immediate(UNICODE_DIGIT)));

const commaSep1 = (rule: RuleOrLiteral) => {
  return prec.left(seq(rule, repeat(seq(token.immediate(/\s*,\s*/), rule))));
};

const commaSep = (rule: RuleOrLiteral) => {
...
const xpathGrammar = grammar({
  ...
  rules: {
    ...
    function_call: ($) => prec(137, seq(
      field('name', prec.left(10, seq($._EQName, token.immediate(/\s*\(\s*/)))),
      optional(field('args', prec.left(137, seq(
        commaSep($._argument),
      )))),
      /\s*\)\s*/
    )),
    ...
    _postfix_expr: ($) => repeat1(choice($.predicate, $.postfix_lookup)), // 49

    predicate: ($) => prec(20, seq('[', field('filter', $._expr), ']')), //124
    // 3.2.2 Dynamic Function Calls
    arg_list: ($) => prec(200, seq(token('('), commaSep($._argument), token(')'))), // 122
    _argument: ($) => prec.left(19, field('argument', choice($.function_call, $._expr, '?'))), // 138
    ...
    path_expr: ($) =>
      prec.left(
        ...
        field('path',
          choice(
            seq(token('/'), optional($._relative_path_expr)), //parse-note-leading-lone-slash
    ...
    _relative_path_expr: ($) =>
      prec.left(109, field('step', seq($.step_expr, optional(repeat1(seq(choice(token.immediate('/'), token.immediate('//')), $.step_expr)))))), //109
    ...
    _step: ($) =>
      prec.left(choice(
        seq(
          field('axis_movement', choice($.forward_axis, $.reverse_axis, optional($.abbrev_foward))),
          field('node_test', $._node_test)
        ),
        $.abbrev_reverse
      )),
    abbrev_foward: (_$) => '@', // 117
    abbrev_reverse: (_$) => '..', // 117
    forward_axis: (_$) =>
      prec.left(seq(
        choice(
          'child',
    ...
    reverse_axis: (_$) => prec.left(seq(choice('parent', 'ancestor', 'preceding-sibling', 'preceding', 'ancestor-or-self'), token.immediate('::'))), //116
    _node_test: ($) =>
      prec.left(23,
        choice(
    ...
    infix_expression: ($) => prec.left(24, seq(
      field('lhs', $._expr),
      prec.left(24, repeat1(seq(
        field('operator', seq(
          repeat(prec.left(2, $._whitespace)),
          $._infix_operator,
          repeat(prec.left($._whitespace)),
        )),
        field('rhs', $._expr)
      )))
    )),
    ...
    _infix_operator: ($) => choice(
      ...
      prec.left(19, seq(
        choice(
          token.immediate(/\s+\*\s+/),
          token.immediate(/\s+div\s+/),
          token.immediate(/\s+idiv\s+/),
          token.immediate(/\s+mod\s+/)
        ),
      )),
      prec.left(18, choice(
        token(/\s*\+\s*/),
    ...
    _comparison_word: () => prec.left(210, choice(
      token.immediate(' and '),
      token.immediate(' or '),
      token.immediate(' eq '),
      token.immediate(' ne '),
    ...
    char_group: (_$) => token(prec.left(repeat1(/[^`\]]/))), // TODO
    ...
    _key_specifier: ($) => choice($.NCName, $.lookup_digit, $.parenthesized_expr, prec.left(alias(token.immediate('*'), $.wildcard)), alias(token('*'), $.wildcard)), // 54
    ...
    if_expr: ($) =>
      prec(
        ...
        seq(
          'if',
          field('if_test', seq(token('('), $._expr, token(')'))),
          'then',
    ...
    any_kind_test: (_$) => seq('node', '(', ')'), // 189
    text_test: (_$) => seq('text', '(', ')'), // 191
    comment_test: (_$) => seq('comment', '(', ')'), // 192
    namespace_node_test: (_$) => seq('namespace-node', '(', ')'), // 193
    ...
    attribute_test: ($) =>
      seq(
        'attribute',
        '(',
        optional(seq($._attrib_name_or_wildcard, optional(seq(',', field('type_name', $._EQName))))),
        ')'
      ), // 195
    ...
    wildcard: ($) => prec.left(choice('*', seq($.NCName, ':*'), seq('*:', $.NCName))), // 120
